...
contract SafuuX is ERC1155, Ownable {
  ...
  bytes32 immutable public _merkleRoot;
  ...
  constructor(string memory name_, string memory symbol_, bytes32 merkleRoot_) ERC1155("") {
    _name = name_;
    _symbol = symbol_;
    _merkleRoot = merkleRoot_;
  }

	function mintFullNode(uint _amount, bytes32[] calldata merkleProof) external {
    require(_amount < 2, "Max 1 Full Node tokens allowed per Whitelist address");
    require(FULL_NODE_CURRENT_SUPPLY + _amount <= FULL_NODE_LIMIT, "Purchase would exceed max Full Node supply");
    require(fullNodeClaimed[msg.sender] == false, "Token already claimed by this address");
    
    bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
    require(_checkEligibility(merkleProof, leaf) == true, "Address not eligible - Invalid merkle proof");
  ...
  function _checkEligibility(bytes32[] calldata merkleProof, bytes32 leaf) internal view returns(bool){
  ...
  function withdrawERC20(IERC20 tokenContract, address to, uint256 amount) external onlyOwner {
