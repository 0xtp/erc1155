...
contract SafuuX is ERC1155, Ownable {
    ...
    uint256 public MAX_GOLDLIST_LIMIT = 20;
    ...
    function mintFullNode(bytes32[] calldata merkleProof) external {
        ...
        require(
            fullNodeClaimed[msg.sender] == false,
            "Token already claimed by this address"
        );

        bytes32 leaf = keccak256(abi.encodePacked(msg.sender));
        require(
            _checkEligibility(_whiteListMerkleRoot, merkleProof, leaf) == true,
    ...
    function mintLiteNode(uint256 _amount, bytes32[] calldata merkleProof)
        external
    {
        require(
        ...
        require(
            liteNodeClaimed[msg.sender] == false,
            "Token already claimed by this address"
        );
        ...
        if (balanceOf(msg.sender, 2) + _amount == 5) {
